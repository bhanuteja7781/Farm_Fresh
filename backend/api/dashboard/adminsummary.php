...
$customersQuery = $conn->query("SELECT COUNT(*) AS count FROM users WHERE role = 'customer'");
$total_customers = $customersQuery->fetch_assoc()['count'] ?? 0;
$farmersQuery = $conn->query("SELECT COUNT(*) AS count FROM users WHERE role = 'farmer'");
$total_farmers = $farmersQuery->fetch_assoc()['count'] ?? 0;
...
$platform_revenue = round((float)$total_revenue * 0.075, 2);
...
$monthly_platform_revenue = round((float)$monthly_revenue * 0.075, 2);
...
echo json_encode([
    ...
    "data" => [
        ...
        "gross_sales" => round((float)$total_revenue, 2),
